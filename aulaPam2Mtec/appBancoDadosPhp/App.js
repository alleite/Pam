...
export default function App() {
  const api = 'http://10.68.36.119/2mtec/apireact/';
  ...
  else if (res.data.success == "Email já cadastrado!") {
    mensagemDuplicidade();
  }
  listarDados();
  setAbrir(false);
  ...
  return (
    <View>
      <View style={styles.navbar}>
        <Text style={styles.textonavbar}>Lista de Usuários
        </Text>
        <TouchableOpacity
          style={styles.botao}
        ...
        <View style={styles.grid}>
          {lista.map(item =>
            <View style={styles.griditem} key={item.id}>
              <Text style={{ color: '#585858' }}> {item.id} - {item.nome} - {item.email}
              </Text>
            </View>
          )}
        </View>
